...
class LDSR():
    def __init__(self, modelPath=None, model=None, torchdevice=None, on_progress=None, yamlPath=path.join(path.dirname(__file__), "config.yaml")):
    ...
    @staticmethod
    def load_model_from_path(modelPath, device, yamlPath=path.join(path.dirname(__file__), "config.yaml")):
